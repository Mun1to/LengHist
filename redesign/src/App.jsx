...
export default function App() {
  ...
  const [resFavs, toggleResFav] = useFavorites()
  const [resFavOnly, setResFavOnly] = useState(false)

  // --- Conceptos ---
  const [conQuery, setConQuery] = useState('')
  const [conCat, setConCat] = useState('all')
  const [conFavs, toggleConFav] = useFavorites()
  const [conFavOnly, setConFavOnly] = useState(false)

  // --- Componentes ---
  const [compQuery, setCompQuery] = useState('')
  const [compCat, setCompCat] = useState('all')
  const [compFavs, toggleCompFav] = useFavorites()
  const [compFavOnly, setCompFavOnly] = useState(false)
  ...
  }, [filter, query, langFavOnly, langFavs])

  const resGroups = useMemo(() => {
    const q = resQuery.trim().toLowerCase()
    return RESOURCES
      .filter((g) => resCat === 'all' || g.key === resCat)
      .map((g) => ({
        ...g,
  ...
  const conGroups = useMemo(() => {
    ...
    return CONCEPTS
      .filter((g) => conCat === 'all' || g.key === conCat)
      .map((g) => ({
        ...g,
  ...
  const toggleCompare = (name) =>
    setCompareSet((prev) => { const n = new Set(prev); n.has(name) ? n.delete(name) : n.add(name); return n })
  ...
  const sidebarProps = useMemo(() => {
    if (activeNav === 'languages') {
      ...
      return {
        searchPh: t.searchPh,
        query, setQuery,
        categories: CATEGORIES.map((c) => ({ key: c.key, label: c.label, count: c.count, dot: c.dot })),
        activeCat: langFavOnly ? null : activeCat,
        setActiveCat: (key) => { setFilter(key === 'all' ? { type: 'all' } : { type: 'cat', value: key }); setLangFavOnly(false) },
        extraGroup: {
          showFavOnly: langFavOnly,
          onToggleFavOnly: () => setLangFavOnly((v) => !v),
          favCount: langFavs.size,
    ...
    if (activeNav === 'resources') {
      return {
        searchPh: t.searchPh,
        query: resQuery, setQuery: setResQuery,
        categories: [
          { key: 'all', label: t.all, count: RESOURCES.reduce((n, g) => n + g.items.length, 0), dot: '#818cf8' },
        ...
        extraGroup: {
          showFavOnly: resFavOnly,
          onToggleFavOnly: () => setResFavOnly((v) => !v),
          favCount: resFavs.size,
        },
      }
    }
    if (activeNav === 'concepts') {
      return {
        searchPh: t.searchPh,
        query: conQuery, setQuery: setConQuery,
        categories: [
          { key: 'all', label: t.all, count: CONCEPTS.reduce((n, g) => n + g.items.length, 0), dot: '#818cf8' },
        ...
        extraGroup: {
          showFavOnly: conFavOnly,
          onToggleFavOnly: () => setConFavOnly((v) => !v),
          favCount: conFavs.size,
        },
      }
    }
    return {
      searchPh: t.searchPh,
      query: compQuery, setQuery: setCompQuery,
      categories: [
        { key: 'all', label: t.all, count: COMPONENT_ITEMS.length, dot: '#818cf8' },
        ...COMPONENT_GROUPS.map((g) => ({
          key: g.key, label: g.label[lang], dot: g.dot,
      ...
      extraGroup: {
        showFavOnly: compFavOnly,
        onToggleFavOnly: () => setCompFavOnly((v) => !v),
        favCount: compFavs.size,
      },
    }
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50">
      <TopBar
        ...
        onQuizClick={goToLanguagesAndScroll}
      />
      <div className="flex">
        ...
        <main className="flex-1 min-w-0 overflow-hidden">
          <AnimatePresence mode="wait">
            ...
            >
              {activeNav === 'languages' && (
                <>
                  <Hero t={t} filter={filter} setFilter={setFilter} onExplore={() => document.getElementById('grid')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} />
                  <LanguageGrid
                    t={t}
                    list={visible}
                    ...
                    favorites={langFavs}
                    onToggleFav={toggleLangFav}
                    compareSet={compareSet}
                    onToggleCompare={toggleCompare}
                    onClearFilters={() => { setFilter({ type: 'all' }); setQuery(''); setLangFavOnly(false) }}
                  />
                  {visible.some((l) => l.name === selected) && <DetailPanel t={t} selected={selected} />}
                </>
              )}
              {activeNav === 'resources' && (
                <ResourcesView
                  t={t} lang={lang} groups={resGroups}
                  favorites={resFavs} onToggleFav={toggleResFav}
                  onClear={() => { setResQuery(''); setResCat('all'); setResFavOnly(false) }}
                />
              )}
              {activeNav === 'concepts' && (
                <ConceptsView
                  t={t} lang={lang} groups={conGroups}
                  favorites={conFavs} onToggleFav={toggleConFav}
                  onClear={() => { setConQuery(''); setConCat('all'); setConFavOnly(false) }}
                />
              )}
              {activeNav === 'components' && (
                <ComponentsView
                  t={t} lang={lang} items={compItems}
                  favorites={compFavs} onToggleFav={toggleCompFav}
