...
import { paintFallbackSnapshot } from "./htmlFallback";
...
export function createCloth(
  ...
  // Respaldo cuando el navegador no trae html-in-canvas: pintamos el HTML a mano
  // en el canvas fuente (con el renderizador del propio canvasui) y lo subimos
  // como textura. El HTML original se esconde con un filtro, porque el
  // renderizador lee `visibility` y `opacity` para decidir que pinta.
  captureFallback = () => {
    if (htmlInCanvas) return;
    try {
  ...
  // Con el respaldo hay que repintar cuando el contenido termina de cargar:
  // no existe el onpaint del navegador que avise.
  const fallbackTimers: number[] = [];
  if (!htmlInCanvas) {
    content.querySelectorAll("img").forEach((img) => {
      if (!img.complete) {
        img.addEventListener("load", () => captureFallback(), { once: true });
      }
    });
    fallbackTimers.push(
      window.setTimeout(() => captureFallback(), 120),
      window.setTimeout(() => captureFallback(), 600),
    );
  }
  ...
  return {
    ...
    destroy() {
      ...
      if (!htmlInCanvas) content.style.filter = "";
